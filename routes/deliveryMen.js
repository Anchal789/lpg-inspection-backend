...
router.post(
	...
	asyncHandler(async (req, res) => {
		...
		if (!productId || !quantity || !price || !minPrice) {
			return sendError(
				res,
				"Product ID, quantity, price, and minPrice are required",
				400
			);
		}
...
router.put(
	"/:id/assigned-products/:assignedProductId",
	requireAdmin,
	asyncHandler(async (req, res) => {
		const { id, assignedProductId } = req.params;
		const { quantity, price, minPrice, originalQuantity, productId, name } = req.body;

		console.log(
			`📝 Updating assigned product ${assignedProductId} for delivery man ${id}`
		);
		...
		if (
			...
			return sendError(res, "All fields are required for update", 400);
		}
		...
		// Find the assigned product
		const assignedProductIndex = deliveryMan.assignedProducts.findIndex(
			(ap) => ap._id.toString() === assignedProductId
		);

		if (assignedProductIndex === -1) {
			return sendError(res, "Assigned product not found", 404);
		}

		// Find the main product to update stock
		const product = await Product.findById(productId);
		if (!product) {
			return sendError(res, "Product not found", 404);
		}
		...
		const availableStock = product.quantity + quantityDifference;
		if (availableStock < 0) {
			return sendError(
				res,
				`Not enough stock available. Maximum quantity: ${
		...
		// Update product stock (add back the difference)
		product.quantity += quantityDifference;
		await product.save();

		// Update the assigned product
		deliveryMan.assignedProducts[assignedProductIndex].quantity = quantity;
		deliveryMan.assignedProducts[assignedProductIndex].price = price;
		deliveryMan.assignedProducts[assignedProductIndex].minPrice = minPrice;
		deliveryMan.assignedProducts[assignedProductIndex].name = name;

		await deliveryMan.save();

		console.log(
			`✅ Assigned product updated for delivery man ${deliveryMan._id}`
		);
		return sendSuccess(
			res,
			{ deliveryMan },
...
router.get(
	...
	asyncHandler(async (req, res) => {
		...
		return sendSuccess(
			res,
			{ deliveryMan: deliveryManDetails },
...
router.post(
	...
	asyncHandler(async (req, res) => {
		...
		if (existingDeliveryMan) {
			return sendError(
				res,
				"Phone number already registered for this distributor",
				400
			);
		}
...
router.put(
	...
	asyncHandler(async (req, res) => {
		...
		console.log("🔄 Toggling status for delivery man:", id);
